...
for i,r in enumerate(booths):
    old=np.array(r['position']);new=np.array([81+2.7*i,F3,63])
    ...
    for xx in [a,b]:box('Interior_ImmigrationPassageReaders',FRAME,(xx,F3+.5,63),(.16,1,.4))
cursor=48.53
for a,b,_ in gaps:
    wall('Interior_ImmigrationControlledBoundary',(cursor,63),(a,63),F3,3,ARR_GLASS);cursor=b
wall('Interior_ImmigrationControlledBoundary',(cursor,63),(111,63),F3,3,ARR_GLASS)
